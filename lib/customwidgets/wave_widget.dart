...
class WaveWidget extends StatefulWidget {
  ...
  WaveWidget(this.size, this.borderColor, this.fillColor, this.progress);

  @override
  WaveWidgetState createState() => new WaveWidgetState();
}

class WaveWidgetState extends State<WaveWidget>
...
class WaveProgressPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    // draw small wave
    Paint wave2Paint = new Paint()..color = fillColor.withOpacity(0.84);
    ...
    for (double i = 0.0; i < size.width; i++) {
      path.lineTo(
          i,
          baseHeight +
              sin((i / size.width * 2 * pi-10 * n) +
                  (_animation.value * 2 * pi-10) +
                  pi * 1) *
                  amp);
    }
